...
def get_financial_prediction(financial_data: Dict[str, Any], n_years: int = 3) -> Dict[int, Dict[str, Any]]:
    try:
        ...
        repo_id = "Writer/Palmyra-Fin-70B-32K"

        llm = HuggingFaceEndpoint(
            repo_id=repo_id,
        )
        chat = ChatHuggingFace(llm=llm, temperature=1)
        parser = PydanticOutputParser(pydantic_object=FinancialAnalysis)
...
def get_financial_prediction_list_hf(CD_CVM_list: List[int], n_years: int=None) -> pd.DataFrame:
    """

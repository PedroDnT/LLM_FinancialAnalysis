...
def upload_predictions(cd_cvm_list: List[int], n_years: Optional[int] = None):
    try:
        ...
        total_uploaded = 0
        for batch_df in get_financial_prediction_list(cd_cvm_list, n_years):
            if batch_df.empty:
                print("No predictions generated in this batch.")
                continue
            
            print(f"Batch predictions generated. Shape: {batch_df.shape}")
            ...
            print(f"\nInserted/updated {num_uploaded} rows in this batch.")
            
            # Update progress bar
            progress_bar.update(len(processed_df['CD_CVM'].unique()))
        
        # Close the progress bar
        progress_bar.close()
        
        print(f"\nTotal inserted/updated rows: {total_uploaded}")
